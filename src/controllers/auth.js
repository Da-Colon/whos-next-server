...
const login = async ( req, res ) => {
  try {
    ...
    if(response.error) return res.status(response.error.httpCode || 500).json({ error: response.error.message })
...
const auth = async ( req, res ) => {
  try {
    ...
    if(response.error) return res.status(response.error.httpCode || 500).json({ response })
    req.user = response
